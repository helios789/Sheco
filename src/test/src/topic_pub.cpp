#include "ros/ros.h"
// ROS 기본 헤더파일
#include "test/MsgTutorial.h"// MsgTutorial 메시지 파일 헤더(빌드 후 자동 생성됨)
int main(int argc, char **argv)
{
    ...
    ros::Publisher ros_tutorial_pub = nh.advertise<test::MsgTutorial>("ros_tutorial_msg", 100);
    // 루프 주기를 설정한다. "10" 이라는 것은 10Hz를 말하는 것으로 0.1초 간격으로 반복된다
    ros::Rate loop_rate(10);
    // MsgTutorial 메시지 파일 형식으로 msg 라는 메시지를 선언
    test::MsgTutorial msg;
    // 메시지에 사용될 변수 선언
    int count = 0;
